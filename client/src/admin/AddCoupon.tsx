...
import { createCoupon } from '../core/helper/couponHelper';
...
const AddCoupon = () => {
  ...
  const onSubmit = (event: React.FormEvent) => {
    ...
    createCoupon(user._id, token, couponData)
      .then(data => {
        if (data?.error) {
          setError(data.error);
          setSuccess(false);
  ...
  const successMessage = () => {
    return (
      ...
      >
        <h4 className="font-semibold">Coupon created successfully!</h4>
  ...
  return (
    <Base title="Create Coupon" description="Add a new discount coupon">
      <Link 
        to="/admin/dashboard" 
