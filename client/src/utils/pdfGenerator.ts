  ...
  static async downloadHTMLAsPDF(
    ...
    try {
      ...
      if (canvas.width === 0 || canvas.height === 0) {
        throw new Error(
          "Generated canvas is empty - HTML content may not have rendered properly",
        );
      }
  ...
  static async downloadInvoiceFromServer(
    ...
    try {
      ...
      if (!contentType.includes("text/html")) {
        ...
        console.error(
          "Unexpected Content-Type from invoice endpoint:",
          contentType,
          errorBody,
        );
        throw new Error(
      ...
      if (error.message.includes("Server error: 404")) {
        throw new Error(
          "Invoice not found. Please try again in a few minutes or contact support.",
        );
      } else if (error.message.includes("Server error: 500")) {
        throw new Error(
          "Invoice generation error. Please try again or contact support.",
        );
      } else {
  ...
  static async smartPDFDownload(
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          "Unable to generate PDF. Please try refreshing the page or using a different browser.",
        );
      }
    }
  }
